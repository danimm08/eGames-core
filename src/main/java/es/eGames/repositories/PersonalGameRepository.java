...
public interface PersonalGameRepository extends JpaRepository<PersonalGame, Integer> {
    ...
    @Query("select pg from PersonalGame pg where pg.user.id = ?1 and pg.exchange.eventDate is null")
    List<PersonalGame> findSemiAvailablePersonalGameByUser(int id);
    ...
    @Query("select pg from PersonalGame pg where pg.exchange is null and pg.user.userAccount.username not like ?1")
    ...
    @Query("select pg from PersonalGame pg where pg.exchange is null and pg.user = ANY (select fw from User u join u.followees fw where u.id = ?1 )")
